...
class Restart(Scene):
    def construct(self):
        ...
        # Highlight MH and HN in the diagram (color, not equation) and pass to arrange_final
        ...
        self.arrange_final(
            ...
            diameter, new_H, new_M, new_L, dot_L, dot_N, line_LN, label_a, line_HM,
            mh_line, hn_line
        )
    ...
    def mark_and_label_radius(self, L, M, T, N, label_c, M_actual, N_actual):
        ...
        # TM (MT) - put c label in vertical orientation with MH (a) and/or HN (c-a)
        ...
        # Vertically align c label with MH (a)
        mt_vec = T - M
        perp_mt = np.array([-mt_vec[1], mt_vec[0], 0])
        perp_mt /= np.linalg.norm(perp_mt)
        offset = 0.18 * perp_mt  # Same as used for 'a'

        tm_mid = (T + M) / 2
        c_on_mt_pos = tm_mid + offset

        c_dup = label_c.copy()
        self.play(c_dup.animate.move_to(c_on_mt_pos).scale(0.85 * 0.24 / label_c.height))
        self.wait(1)
        ...
        # Animate "c" flying from LM (midpoint) to the curly brace
        lm_mid = (L + M) / 2
    ...
    def arrange_final(
        ...
        diameter, new_H, new_M, new_L, dot_L, dot_N_real, line_LN, label_a_obj, line_HM,
        mh_line, hn_line
    ):
        ...
        # Remove the old equation group from the scene (no fade out/in for equation)
        ...
        # (12) Fade out HM from the diagram
        self.play(FadeOut(line_HM))
        self.wait(0.5)

        # (13) Make MH in the diagram white again
        self.play(mh_line.animate.set_color(WHITE))
        self.wait(0.2)

        # (14) Make HN in the diagram white again
