...
class Restart(Scene):
    ...
    def arrange_final(
        ...
        # Place "Let's let ..." line
        instruction = MathTex(
            r"\text{Let's let}\ \angle LHM = 2x", font_size=LABEL_FONT_SIZE_SMALL
        ...
        # Fade out the c label on LM before writing the question marks
        self.play(FadeOut(label_c_lm))
        self.wait(0.3)
        ...
        # Fade out LM, c on LM, q2, and 2x
        self.play(
            FadeOut(lm_line),      # Green LM segment
            FadeOut(label_c_lm),   # c label on LM
            FadeOut(q2),           # x at q2
