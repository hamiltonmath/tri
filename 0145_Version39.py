...
class Restart(Scene):
    def construct(self):
        ...
        # 2. Draw right angle mark at H between M and L, initially large
        ...
        # Shrink the initial right angle mark at H to match TLN's style and position
        def get_shrunken_right_angle_pos(H, M, L, size):
            v1 = (M - H) / np.linalg.norm(M - H)
            v2 = (L - H) / np.linalg.norm(L - H)
            bisector = (v1 + v2) / np.linalg.norm(v1 + v2)
            return H + bisector * size * 0.8
        target_pos = get_shrunken_right_angle_pos(new_H, new_M, new_L, RIGHT_ANGLE_SIZE)
        shrink_scale = RIGHT_ANGLE_SIZE / INITIAL_RIGHT_ANGLE_SIZE
        self.play(right_angle_mark.animate.scale(shrink_scale).move_to(target_pos))
        self.wait(0.3)
        ...
        self.arrange_final(
            ...
            diameter, new_H, new_M, new_L, dot_L, dot_N, line_LN, label_a
        )
    ...
    def arrange_final(self, remain_group, dot_M, dot_H, dot_N, label_a, label_c_on_mn, c_above_brace, brace, diameter, new_H, new_M, new_L, dot_L, dot_N_real, line_LN, label_a_obj):
        ...
        # (1) Place equation centered, now half a line higher than center, with color
        eq_mh = MathTex("MH", font_size=EQ_FONT_SIZE, color=HN_LINE_COLOR)  # blue
        eq_plus = Text("+", font_size=EQ_FONT_SIZE, color=WHITE)
        eq_hn = MathTex("HN", font_size=EQ_FONT_SIZE, color=MH_LINE_COLOR)  # red
        ...
        # (2+3) Highlight MH and HN on the diagram (MH blue, HN red)
        ...
        # (4) MH disappears, a label flies in, but a remains in the diagram
        ...
        # (5) Circle c_above_brace
        circ_c = Circle(0.32, color=HIGHLIGHT_COLOR).move_to(c_above_brace.get_center())
        self.play(Create(circ_c))
        self.wait(0.3)

        # (6) MN disappears, c_above_brace uncircled, then c_above_brace flies in (and stays as c, no swap back)
        ...
        # Remove the old equation group from the scene
        self.remove(eq_group, eq_mh, eq_mn, eq_plus, eq_hn, eq_eq)

        # (8) Keep the equation "a + HN = c", keep a blue, HN red for now
        equation_a = label_a_fly  # blue
        equation_plus = Text("+", font_size=EQ_FONT_SIZE, color=WHITE)
        equation_hn = MathTex("HN", font_size=EQ_FONT_SIZE, color=MH_LINE_COLOR) # red
        ...
        # (9) Show deduction HN = c - a (normal spacing)
        ...
        # (10) Diagram label for HN (c-a, tight, below segment, in white)
        ...
        # (11) Morph MH and HN in the equation to white at the END
        self.play(
